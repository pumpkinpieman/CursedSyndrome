...
::CPU
Set CPUName= CPU Processor
set CPUDefaultStartAmount=0
Set CPUSizeInInventory=1
...
set EnemyDT=%enemy_FNAME% %enemy_LNAME%
...
if %input0% equ 2 GOTO LoadPreviousPlayer
...
echo.
echo.
Echo What's your name soldier?
echo.
set /p Fname=First Name: 

:PlayerNamePart2
set /p Lname=Last Name: 
if [%Lname%]==[%BossName1%] ( ECHO You can't use that name! )
set playerdt=%fname% %lname%
echo. %playerdt%>> %fname%%lname%.syndrome
GOTO Class
...
ECHO 2. USEC
set /p Class=Choice: 
if [%class%]==[1] set class=%class1%
if [%class%]==[2] set class=%class2%
ECHO. %Class% >> %fname%%lname%.syndrome
::Class catch for errors or blank class
if [%class%]==[%classblank%] ( GOTO :Class ) ELSE ( goto BagSelection )
...
:GameStart
cls
echo. %PlayerBagChoice%>> BagInventory%fname%%lname%.Syndrome
...
if [%PlayerBagChoice%]==[1] Set PrimaryBag=Berkut 
if [%PlayerBagChoice%]==[1] Set BagInventorySize=32
if [%PlayerBagChoice%]==[1] set BagInventorySizeMax=32

if [%PlayerBagChoice%]==[2] Set PrimaryBag=Pilgrim
if [%PlayerBagChoice%]==[2] Set BagInventorySize=64
if [%PlayerBagChoice%]==[2] set BagInventorySizeMax=64

echo. %PlayerBagChoice% >> %fname%%lname%.Syndrome

if %PrimaryBag% equ Berkut ( echo. Berkut Backpack >> %fname%%lname%.syndrome 
) else ( 
echo. Pilgrim Backpack >> %fname%%lname%.syndrome )
...
TYPE InventoryItemsList%fname%%lname%.Syndrome
GOTO :Loadcommand
...
type baginventory%fname%%lname%.Syndrome
goto Loadcommand
...
type baginventory%fname%%lname%.Syndrome >> InventoryItemsList%fname%%lname%.Syndrome
del InventoryItemsList%fname%%lname%.Syndrome
ECHO. >> baginventory%fname%%lname%.Syndrome

if [%PrimaryBag%]==[Berkut]	 Set BagInventorySize=32
if [%PrimaryBag%]==[Pilgrim] Set BagInventorySize=64
...
echo No previous players exist - Start a new game
Pause
cls
goto MainScreen
)
...
echo.
echo.
set /p PlayerProfileSelect=Select Your Profile:
if [*.syndrome] exist (
	type *%fname%%lname%.syndrome )
	else (
	echo OOPS )
	
if [%PlayerProfileSelect%] equ [*%fname%%lname%*] ( goto MainMenu ) 
	else ( echo oops )
	Pause
...
ECHO. %PRIMARYWEAPON%>> baginventory%fname%%lname%.syndrome
...
if [%PrimaryWeaponMagazineCountQuestion%]==[1] set /a BagInventorySize=%BagInventorySize%=-2
if [%PrimaryWeaponMagazineCountQuestion%]==[2] set /a BagInventorySize=%BagInventorySize%=-4
...
ECHO. %SecondaryWeapon%>> baginventory%fname%%lname%.syndrome

if [%SecondaryWeaponMagazineCountQuestion%]==[1] set /a BagInventorySize=%BagInventorySize%=-2
if [%SecondaryWeaponMagazineCountQuestion%]==[2] set /a BagInventorySize=%BagInventorySize%=-4
...
echo 1. Yes - Heal!
echo 2. No! - Main Menu
set /p HealthBillQuestion=Do you want to heal completely for 10R?
if %HealthBillQuestion% equ 1 goto HealAccept
if %HealthBillQuestion% equ 2 goto GameStart
::----------------------------------------------------------------------------------------------
:HealAccept
set /a R=%R-10
...
if [%BagInventorySize%] NEQ [%BagInventorySizeMax%] ( ECHO You Don't have enough inventory space for a new run! ) ELSE ( echo WHAT NOW?! )
...
echo.
echo.
if exist DogtagCase%fname%%lname%.Syndrome (
	type DogtagCase%fname%%lname%.Syndrome ) else (
	echo. No Dogtags have been collected yet! )
...
cls
type GameOver.txt
del baginventory%fname%%lname%.Syndrome
set hp=3
set water=8
set food=4
set energy=12
...
echo. %BANDAGE%>> baginventory%fname%%lname%.Syndrome
echo. %LUNCHBOX%>> baginventory%fname%%lname%.Syndrome
...
echo. %HK16% >> baginventory%fname%%lname%.Syndrome
...
:InterchangePart5
echo. %BossName1% was Killed By: %PlayerDT%>> DogtagCase%fname%%lname%.Syndrome
echo You find Killa's Dogtag, Helmet, and his RPK-16 full of BT ammo, 2 extra 90 round magazines, Killa's Dog Tag, and 6 grenades.
echo.
echo.
